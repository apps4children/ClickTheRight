...
{ 
    ...
    { 
        ...
        initialValues.put("time1","");
        initialValues.put("time2","");
        initialValues.put("time3","");
        initialValues.put("totaltime","0");

        long recordID=mDb.insert(TABLE_NAME, null, initialValues);
        id= Long.toString(recordID);
        
        System.out.println("IDDDDDDDDD: "+recordID);
    ...
    { 
        ...
        try 
        { 
        	String sql ="select totaltime from score where _id="+id; 
        	Cursor cursor = mDb.rawQuery(sql,null); 
            if(cursor.moveToFirst()){
           	   totaltime=Integer.parseInt(cursor.getString(cursor.getColumnIndex("totaltime")))+time;
            }
            cursor.close(); 
            
            ContentValues initialValues = new ContentValues();
            initialValues.put("totaltime",String.valueOf(totaltime));
            mDb.update(TABLE_NAME, initialValues, "_id=" + id, null);
        } 
        catch (SQLException mSQLException){ 
            throw mSQLException; 
